...
import os  # for use in determining valid file names (respond)
...
def respond(sock):
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        # will not be checking this test case
        """
        doc_name = url.split("/")
        doc_name = doc_name[len(doc_name) - 1]
        if not doc_name.endswith(("html", "css")):  # has to end with one
            transmit(STATUS_FORBIDDEN, sock)
        """

        # check if file exists in directory
        DOCROOT = "./pages"  # root directory to look for files within
        url = DOCROOT + url
        file_exists = os.path.exists(url)
        if file_exists:
            # transmit(STATUS_OK, sock)
            with open(url, 'r', encoding='utf-8') as source:
                for line in source:
                    transmit(line, sock)
        else:
