...
def example():
    ...
    ###################################################
    # the following code is what some may consider 
    ...
    wrd = {"word": str(text)}
    lst = {"in_list": str(text) in WORDS.as_list()}
    ltrs = {"letters": flask.session["jumble"]}
    
    # checks if word is in jumble
    check = True
    for letter in text:
        if not letter in flask.session["jumble"]:
            check = False        
    jmble = {"in_jumble": check}
    
    # checks if word was already found
    mtch = {"in_matched": False}
    matched = flask.session.get("matches", [])
    if text in matched:
        mtch["in_matched"] = True
    elif jmble["in_jumble"] and lst["in_list"] and not (text in matched):
        matched.append(text)
        flask.session["matches"] = matched
    
    # leave if 3 matches are found
    dne = {"is_done": False}
    if len(matched) >= flask.session["target_count"]:
        dne["is_done"] = True
    

    return flask.jsonify(word=wrd, bools=lst, jumble=jmble, letters=ltrs, match=mtch, done=dne)
